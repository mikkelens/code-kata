...
fn chop_stack_recursive(target: usize, sorted_array: &[usize]) -> Option<usize> {
	fn chop_recursive(
		...
		match hit.cmp(&target) {
			...
			Ordering::Less => {
				chop_recursive(target, sorted_array, array_search_index..search_range.len())
			},
...
mod tests {
	use super::*;
	#[test]
	fn test_flat_iterative() { test_implementation(chop_flat_looping); }
	#[test]
	fn test_stack_recursive() { test_implementation(chop_stack_recursive); }
	struct Query {
		target: usize,
		array:  &'static [usize]
	}
	struct TestCase {
		expected_result_index: Option<usize>,
		search:                Query
	}
	fn test_implementation(implementation: fn(usize, &[usize]) -> Option<usize>) {
		for case in TEST_CASES {
			assert_eq!(
				case.expected_result_index,
				implementation(case.search.target, case.search.array)
			);
		}
	}
