...
fn main() {
    ...
    store.scan(&bean_name).expect("there is at least 1 bean can at this point");
    store.scan(&bean_name).expect("there is at least 1 bean can at this point");
    store.scan(&bean_name).expect("there is at least 1 bean can at this point");
    store.scan(&banana_name).expect("there is at least 1 bean can at this point");
    store.scan(&banana_name).expect("there is at least 1 bean can at this point");
    store.scan(&bean_name).expect("there is at least 1 bean can at this point");
    store.scan_multiple(&milk_name, 5).expect("should be able to do this many");
    store.unscan_multiple(&milk_name, 1).expect("there is one to unscan");
    store.scan_multiple(&milk_name, 2).expect("there is space again");
    store.unscan_multiple(&bean_name, 4).expect("removed all at once");
    store.unscan(&bean_name).expect_err("cannot remove another");
    store.scan_multiple(&bean_name, 10).expect("took them all");
    store.scan_multiple(&bean_name, 320).expect_err("can't take this many");
    println!("Checkout at the end: {:?}", store.active_checkout);
}
