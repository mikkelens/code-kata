...
fn part_2() {
	...
	let entry_iter = data_str
		.lines()
		.filter(|line| !line.contains("---") && !line.is_empty())
		.map(|line| line.parse::<TeamStats>().unwrap());

	let for_against_difference_iter = entry_iter.map(|entry| u8::abs_diff(entry.F, entry.A));
...
impl FromStr for TeamStats {
	...
	fn from_str(mut line: &str) -> Result<Self, Self::Err> {
		...
		let (rank, mut line) = line
			.split_once(". ")
			.ok_or("Couldn't split rank from line")?;
		let rank = rank.parse().map_err(|_| "Couldn't parse rank as u8")?;

		line = line.trim_start();
		let (name, mut line) = line
			.split_once(' ')
			.ok_or("Couldn't split name from line")?;
...
fn part_1() {
	...
	let data_str = input
		.split_once('\n')
		.expect("split first time")
		.1
		.split_once('\n')
		.expect("split second time")
		.1;
	...
	let min_difference = temperature_differences
		.iter()
		.min()
		.expect("Could not find min value");
	println!("Min temperature difference: {}", min_difference);
	let max_difference = temperature_differences
		.iter()
		.max()
...
impl FromStr for WeatherDataEntry {
	...
	fn from_str(mut line: &str) -> Result<Self, Self::Err> {
		...
		// dbg!(line);
		let (AvDP, mut line) = line
			.split_once(' ')
			.ok_or("Couldn't split AvDP from line")?;
		...
		line = line.trim_start();
		// dbg!(line);
		let (TPcpn, mut line) = line
			.split_once(' ')
			.ok_or("Couldn't split TPcpn from line")?;
		line = line.trim_start();

		// dbg!(line);
		let (PDir, mut line) = line
			.split_once(' ')
			.ok_or("Couldn't split PDir from line")?;
		line = line.trim_start();

		// dbg!(line);
		let (AvSp, mut line) = line
			.split_once(' ')
			.ok_or("Couldn't split AvSp from line")?;
		line = line.trim_start();
		...
		// dbg!(line);
		let (SkyC, mut line) = line
			.split_once(' ')
			.ok_or("Couldn't split SkyC from line")?;
		line = line.trim_start();
		...
		// dbg!(line);
		Ok(WeatherDataEntry {
			Dy:     Dy.parse().map_err(|_| "Couldn't parse Dy as u8")?,
			MxT:    MxT.parse().map_err(|_| "Couldn't parse MxT as u8 (*)")?,
			MnT:    MnT.parse().map_err(|_| "Couldn't parse MnT as u8 (*)")?,
			AvT:    AvT.parse().map_err(|_| "Couldn't parse AvT as f32")?,
			HDDay:  {
				...
				if HDDay_entry.chars().any(|c| c != ' ') {
					...
					let value = HDDay_entry
						.trim()
						.parse()
						.map_err(|_| "Couldn't parse HDDay entry as u8")?;
					Some(value)
				} else {
					None
				}
			},
			AvDP:   AvDP.parse().map_err(|_| "Couldn't parse AvDP as f32")?,
			OneHrP: ONE_HR_P,
			TPcpn:  TPcpn.parse().map_err(|_| "Couldn't parse TPcpn as f32")?,
			WxType: {
				...
				if WxType_chars.clone().count() > 0 {
					...
					for c in WxType_chars {
						match c {
							'R' => {
								WxType_mut.insert(WxType::R);
							},
							'T' => {
								WxType_mut.insert(WxType::T);
							},
							'F' => {
								WxType_mut.insert(WxType::F);
							},
							'H' => {
								WxType_mut.insert(WxType::H);
							},
			...
			PDir:   PDir.parse().map_err(|_| "Couldn't parse PDir as u16")?,
			AvSp:   AvSp.parse().map_err(|_| "Couldn't parse AvSp as f32")?,
			Dir:    Dir.parse().map_err(|_| "Couldn't parse Dir as u16")?,
			MxS:    MxS.parse().map_err(|_| "Couldn't parse MxS as u8 (*)")?,
			SkyC:   SkyC.parse().map_err(|_| "Couldn't parse SkyC as f32")?,
			MxR:    MxR.parse().map_err(|_| "Couldn't parse MxR as u8")?,
			MnR:    MnR.parse().map_err(|_| "Couldn't parse MnR as u8")?,
			AvSLP:  line.parse().map_err(|_| "Couldn't parse AvSLP as f32")?
		})
	}
}
...
impl FromStr for AsteriskBound<u8> {
	...
	fn from_str(entry: &str) -> Result<Self, Self::Err> {
		...
		Ok(if entry.contains('*') {
			AsteriskBound::Untrustworthy(value)
		} else {
			AsteriskBound::Clear(value)
		})
	}
}
...
impl<T> AsteriskBound<T> {
	fn val(&self) -> &T {
		match self {
			AsteriskBound::Clear(v) | AsteriskBound::Untrustworthy(v) => v
		}
	}
}
