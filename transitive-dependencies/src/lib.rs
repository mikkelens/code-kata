...
pub fn expand_dependencies(map: &mut BTreeMap<Item, Dependencies>) {
    fn gather_dependencies_recursive(source: &Item, target: &Item, map: &BTreeMap<Item, Dependencies>) -> Dependencies {
        let Some(direct_known_dependencies) = map.get(target) else {
            return HashSet::new();
        };
        let mut more_non_source_dependencies: Dependencies = HashSet::from_iter(
            direct_known_dependencies.iter().filter(|dep| dep != &source).copied()
        );
        for sub_dependency in more_non_source_dependencies.clone() {
            more_non_source_dependencies.extend(gather_dependencies_recursive(source, &sub_dependency, map));
        }
    ...
    for (dependency, sub_dependencies) in map.iter_mut() {
        sub_dependencies.extend(gather_dependencies_recursive(dependency, dependency, &map_unchanged));
    }
}
